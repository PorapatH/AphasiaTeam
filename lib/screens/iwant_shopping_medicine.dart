...
class _WantShoppingMedicineState extends State<WantShoppingMedicine>{
  ...
  void initState() {
    ...
    super.initState();
    readIwshoppingMedicine();
  }

  Future<void> readIwshoppingMedicine() async {
    FirebaseFirestore firestore = FirebaseFirestore.instance;
    var shoppingmedicine = firestore
        .collection('Iwant')
        .doc('av9GjXDgd6HdvekrXdNQ')
        .collection('Shopping')
        .doc('BrEMKzj6gzjVaojSy6AH')
        .collection('Medicine')
        .orderBy('Mdid');

    await shoppingmedicine.snapshots().listen((response) {
      List<DocumentSnapshot> snapshots = response.docs;
      for (var snapshot in snapshots) {
        print('snapshot = $snapshot');
        print('Medicine = ${snapshot['Mdname']}');
  ...
  Widget showText(int index) {
    String iwspmedicine = iwantModels[index].medicine;
        ...
        appBar: new AppBar(
          ...
          leading: IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => WantShoppingMedicine()),
              );
            },
          ),
          title: const Text('Aphasia', style: TextStyle(color: Colors.black)),
                ...
                Row(
                  ...
                  children: <Widget>[
                    ClipRRect(
                      ...
                      child: Image.network(args.medicine_pic,
                ...
                Row(
                  ...
                  children: <Widget>[
                    ElevatedButton(
                      ...
                      onPressed: () {
                        tts.speak('ฉันต้องการซื้อ' + args.medicine);
                      },
                    ),
                  ],
                ),
